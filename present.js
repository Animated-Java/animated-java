;(() => {
	...
	function itterator(arr, path = [], res = {}) {
		...
		arr.forEach((item) => {
			if (item instanceof Group) {
				...
				write(res, `${base}.rot.deg.x`, () =>
					Math.radToDeg(mesh.rotation.x)
				)
				write(res, `${base}.rot.deg.y`, () =>
					Math.radToDeg(mesh.rotation.y)
				)
				write(res, `${base}.rot.deg.z`, () =>
	...
	Plugin.register('present', {
		...
		author: 'FetchBot',
		description:
			'adds many molangs to the Animator.MolangParser.global_variables',
		version: '1.0.0',
		variant: 'both',
		onload() {
