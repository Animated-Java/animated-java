...
execute store success score #success aj.i run function animated_java:test_blueprint2/zzz/19 with storage aj:temp args
$execute unless score #success aj.i matches 1 run return run tellraw @a "The animation $(animation) does not exist."
execute if data storage aj:temp {args:{start_animation: true}} run function animated_java:test_blueprint2/zzz/22 with storage aj:temp args