...
execute unless entity @s[type=item_display,tag=aj.rig_root] run return run \
	...
	{'function_path': 'animated_java:test_blueprint/animations/anim1/play'}
tag @s add aj.test_blueprint.animation.anim1.playing
execute store result storage aj:temp frame int 1 run scoreboard players set @s aj.frame 1
function animated_java:test_blueprint/animations/anim1/zzz/frames/1