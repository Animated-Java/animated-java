...
CustomAction('save_aj_project_as', {
	icon: 'save',
	category: 'file',
...
Action.prototype.patch = function (name) {
	;(this.patches = this.patches || []).push(BarItems[name])
}
Action.prototype._trigger = Action.prototype.trigger
Action.prototype.trigger = function (...args) {
	if (this.patches) {
		const defer = this.patches.find((action) =>
			BARS.condition(action, action)
		)
		if (defer) {
			return defer.trigger(...args)
		}
	}
