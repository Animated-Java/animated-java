<script lang="ts" context="module">
	...
	const TRANSLATIONS = {
		teleported_entity_type: {
			displayName: translate('animated_java.camera_config.teleported_entity_type'),
			description: translate(
				'animated_java.camera_config.teleported_entity_type.description'
			).split('\n'),
			error: {
				unset: translate('animated_java.camera_config.teleported_entity_type.error.unset'),
				space: translate('animated_java.camera_config.teleported_entity_type.error.space'),
				invalid_namespace: translate(
					'animated_java.camera_config.teleported_entity_type.error.invalid_namespace'
				),
			},
			warning: {
				unknown_entity: translate(
					'animated_java.camera_config.teleported_entity_type.warning.unknown_entity'
				),
			},
		},
	}
...
<script lang="ts">
	...
	for (const [id, setting] of Object.entries(settings)) {
		switch (id) {
			default:
				// @ts-ignore
				setting.value = camera[id] === undefined ? setting.defaultValue : camera[id]
				break
		}
	}
