...
export function loadExporter() {
	...
	new API.Exporter({
		...
		name: API.translate('animated_java.exporters.statue_exporter.name'),
		description: API.translate('animated_java.exporters.statue_exporter.description'),
		getSettings() {
			return {
				foo: new API.Settings.CheckboxSetting({
					id: 'animated_java:statue_exporter/foo',
					displayName: API.translate(
						'animated_java.exporters.statue_exporter.settings.foo'
					),
