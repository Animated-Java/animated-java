...
export function getNodeTags(node: AnyRenderedNode, rig: IRenderedRig): NbtList {
	...
	tags.push(
		...
		TAGS.PROJECT_ENTITY(),
		TAGS.PROJECT_NODE(),
		TAGS.PROJECT_NODE_NAMED(node.safe_name)
	)
	...
	switch (node.type) {
		case 'bone': {
			tags.push(
				...
				// Project
				TAGS.PROJECT_DISPLAY_NODE_NAMED(node.safe_name),
				TAGS.PROJECT_BONE(),
				TAGS.PROJECT_BONE_NAMED(node.safe_name),
				TAGS.PROJECT_BONE_TREE(node.safe_name), // Tree includes self
				TAGS.PROJECT_BONE_TREE_BONE(node.safe_name) // Tree includes self
			)
			if (!node.parent) {
		...
		case 'item_display': {
			tags.push(
				...
				TAGS.PROJECT_ITEM_DISPLAY_NAMED(node.safe_name)
			)
			if (!node.parent) {
				// Nodes without parents are assumed to be root nodes
		...
		case 'block_display': {
			tags.push(
				...
				TAGS.PROJECT_BLOCK_DISPLAY_NAMED(node.safe_name)
			)
			if (!node.parent) {
				// Nodes without parents are assumed to be root nodes
		...
		case 'text_display': {
			tags.push(
				...
				TAGS.PROJECT_TEXT_DISPLAY_NAMED(node.safe_name)
			)
			if (!node.parent) {
				// Nodes without parents are assumed to be root nodes
		...
		case 'locator': {
			tags.push(
				...
				TAGS.PROJECT_LOCATOR_NAMED(node.safe_name)
			)
			if (!node.parent) {
				// Nodes without parents are assumed to be root nodes
		...
		case 'camera': {
			tags.push(
				...
				// Project
				TAGS.PROJECT_CAMERA(),
				TAGS.PROJECT_CAMERA_NAMED(node.safe_name)
			)
			if (!node.parent) {
				// Nodes without parents are assumed to be root nodes
